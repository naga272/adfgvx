...
def order(key: str) -> list:
    test = sorted(range(len(key)), key=lambda i: key[i])
    print("vedi test", test)
    return test


def trasposizione(stringa_rimappata: str) -> str:
    ...
    print(
        "keyword originale:\n" +
        "".join(char + "  " for char in key_word) +
        "\n" +
        "".join(f"{ord(char)} " for char in key_word) +
        "\n" +
        f"{keyword_riordinata}"
    )
...
def algoritmo_adfgvx(to_encrypt: str) -> str:
    ...
    *   algoritmo adfgvx step by step
    ...
    stringa_finale = trasposizione(stringa_rimappata)
